(function () {
  ...
  app.get("/cards/:id", function (req, res, next) {
    helpers.simpleHttpRequest(
      endpoints.cardsUrl + "/" + req.params.id,
      res,
      next
    );
  });
  ...
  app.get("/card", function (req, res, next) {
    ...
    request(
      ...
      function (error, response, body) {
        ...
        return helpers.respondSuccessBody(
          res,
          JSON.stringify({ status_code: 500 })
        );
      }.bind({
        res: res,
      })
  ...
  app.post("/register", function (req, res, next) {
    ...
    async.waterfall(
      ...
      function (err, custId) {
        ...
        res.status(200);
        res
          .cookie(cookie_name, req.session.id, {
            maxAge: 3600000,
          })
          .send({ id: custId });
